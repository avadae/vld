...
void AssertCompareCallStacks(const wchar_t* actual,
    const char* expected)
{
    bool succeeded = true;
    ...
    while (std::getline(expectedStream, expectedLine))
    {
        ...
        {
            Assert::Fail(resultStream.str().c_str(), LINE_INFO());
            return;
        }
        ...
        if (!std::regex_match(actualLine, expectedRegex))
        {
            if (succeeded)
            {
                succeeded = false;
                resultStream
                    << actualLine.c_str() << " and " << expectedLine.c_str() << " lines do not match\n";
            }
            resultStream
                << "line:\n" << actualLine.c_str() << "\n and \n" << expectedLine.c_str() << "\n";
        }
